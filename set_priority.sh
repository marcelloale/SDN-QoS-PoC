...
if [ "$3" == "hi" ]; then
    ...
    sudo ovs-ofctl -O OpenFlow10 add-flow core2 priority=10,idle_timeout=60,$flow,actions=set_queue:1,output:2
elif [ "$3" == "lo" ]; then
    echo "sudo ovs-ofctl -O OpenFlow10 del-flows core2 $flow"
    sudo ovs-ofctl -O OpenFlow10 del-flows core2 $flow
